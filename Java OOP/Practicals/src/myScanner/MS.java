 ...
 * It displays information about the data needed - The class has predefined
 * STRINGS and CHARS types to facilitate random data - Int and double data can
 * be limited by the range - Class does not require creating an object (we use
 * like Math) - When the data is bad, he asks for the data to be re-entered -
 * The CHARS class allows you to add your own set of characters - The STRINGS
 * class allows you to add your own set of words - You can add your own
 * predefined values to the STRINGS class (info in the comment)
 *
 * Changes - Order of parametrs in getInt ( info, fill, from, to ) - Same in
 * getDouble ( info, fill, from, to ) like getString and getChar - STRINGS
 * predefined Yes_No and Yes_No_Cancel with validation - Primitive testers for
 * all method
 * 
 * Hereafter getFDouble ( info, fill, rounded, from, to ) random ( rounded,
 * from, to )
 *
...
public class MS {
	...
	public static int getInt(String info, boolean fill, int to) {
		// from zero
		return getInt(info, fill, 0, to);
	}
